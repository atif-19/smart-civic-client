          ...
          {user ? (
            <>
              ...
              >
                ...
                <Link
                  href="/admin"
                  className="text-sm font-semibold bg-indigo-500 hover:bg-indigo-600 px-3 py-2 rounded-md transition-colors"
                >
                  Admin Dashboard
                </Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-4">
          <div className="flex flex-col items-start space-y-4">
            {user ? (
              <>
                <span className="text-sm w-full pb-2 border-b border-slate-700">
                  {user.email} |{" "}
                  <span className="font-bold text-teal-400">
                    {user.points} Points
                  </span>
                ...
                <button
                  ...
                  className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded text-sm w-full text-left"
                >
                  Logout
                </button>
              </>
              ...
              <>
                <Link
                  href="/login"
                  className="hover:text-teal-400 w-full"
                  onClick={() => setIsMenuOpen(false)}
                >
                  Log In
                </Link>
                <Link
                  href="/register"
                  className="bg-teal-500 hover:bg-teal-400 text-slate-900 font-bold py-2 px-4 rounded w-full text-center"
                  onClick={() => setIsMenuOpen(false)}
                >
                  Sign Up
                </Link>
