  ...
  return (
    <>
      <main className="flex flex-col md:flex-row h-screen bg-gray-100 font-sans">
        ...
        {/* --- UPDATED: Map Section is now wrapped in ClientOnly --- */}
        <div
          className={`${
        ...
        <div
          ...
          } md:flex w-full md:w-1/2 h-full flex-col p-3 md:p-6`}
        >
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 flex-shrink-0 space-y-2 sm:space-y-0">
            ...
            <Link
              href="/report"
              className="bg-blue-500 text-white font-bold py-2 px-3 md:px-4 rounded hover:bg-blue-700 transition-colors text-center text-sm md:text-base"
            >
              + New Report
            </Link>
          </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start space-y-3 sm:space-y-0 sm:space-x-4">
                  <button
                    ...
                    className="flex-shrink-0 w-full sm:w-auto flex justify-center sm:justify-start"
                  >
                    <img
                      src={report.imageUrl}
                      alt={report.category}
                      width={96}
                      height={96}
                      className="w-20 h-20 sm:w-24 sm:h-24 object-cover rounded-md cursor-pointer hover:opacity-80 transition-opacity"
                    />
                  </button>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-2 text-sm text-gray-800 leading-relaxed truncate">
                      {report.description}
                    </p>
                  ...
                  <div className="flex-shrink-0 w-full sm:w-auto">
                    <select
                      ...
                      className={`w-full sm:w-auto text-xs font-bold p-2 rounded-md border-2 appearance-none ${getStatusColor(
                        report.status
                      )}`}
                    >
                      <option value="submitted">Submitted</option>
      ...
      {selectedImage && (
        ...
        >
          <button
            className="absolute top-4 right-4 text-white bg-red-500 rounded-full w-8 h-8 flex items-center justify-center text-xl font-bold z-10"
            onClick={() => setSelectedImage(null)}
          >
            &times;
          </button>
